...
void	handle_rectangles_OMPstriping(struct area_t * area);
...
void area_under_curve(struct area_t * area, int argc, char ** argv){  
  ...
  handle_rectangles_OMPstriping(area);
 ...
 ***********************************************************************************************/
/*For OpenMP striping, do not use #pragma omp parallel for, because this defaults to
* blocking. Instead, use #pragma omp parallel, retrive the thread id and use this information
* to perform the for loop.
* NOTE: MPI is still doing blocking. */
void handle_rectangles_OMPstriping(struct area_t * area){
  struct display_t * d = &(area->d);
  struct rec_t * rect = &(area->rect);
  struct curve_t * curve = &(area->curve);
  int current_rectangle=0, id=0;
  double current_left=0.0, current_height=0.0, integral=0.0;
  
  #ifdef OMP
  #pragma omp parallel reduction(+: integral) shared(area,rect,curve) private(current_rectangle, current_left, current_height)
  {
    id = omp_get_thread_num();
  #endif
    for(current_rectangle = id; current_rectangle< rect->private_num_rect; current_rectangle += area->num_threads)
    {
...
/*************************END OF handle_rectangles_OMPstriping()*****************************/
