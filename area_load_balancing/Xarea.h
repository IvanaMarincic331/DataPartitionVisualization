...
void get_color(struct area_t * area){
  struct display_t * d = &(area->d);
  #ifndef NO_X11
  int order = area->num_threads * area->rank + area->id;
  switch(order){
    case RED:
      cred(d);
      break;
...
void draw_ranks(struct area_t * area) {
  ...
  for(id =0; id<area->num_threads; id++){
    area->id = id; //for get_color();
    thread_x = 7+(d->width/4)*area->id; // X coordinate for thread display
    sprintf(thread_string,"%d", area->id);
    // Draw the thread display 
    get_color(area); //get color specific to rank+thread
...
void draw_rectangle(struct area_t * area, double current_left, double current_height) {
  ...
  int rank = area->rank;
  #ifdef OMP
  area->id = omp_get_thread_num(); //for get_color()
  #endif
  double rect_width = area->rect.width;
