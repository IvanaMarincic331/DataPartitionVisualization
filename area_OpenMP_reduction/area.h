...
void	handle_rectangles_reductionOMP(struct area_t * area);
...
void area_under_curve(struct area_t * area, int argc, char ** argv){  
  ...
  handle_rectangles_reductionOMP(area);
 ...
 ***************************************************************************/
/*Introduce a new variable to hold the reduced sum. If OMP is specified, make sure to
store this sum back into the area struct*/
void handle_rectangles_reductionOMP(struct area_t * area){
  struct rec_t * rect = &(area->rect);
  struct curve_t * curve = &(area->curve);
  int current_rectangle;
  double current_left,current_height, integral=0.0;
  
  #ifdef OMP 
  #pragma omp parallel for reduction(+: integral) private(current_rectangle, current_left, current_height)
  #endif
...
/*************************END OF handle_rectangles_reductionOMP()*****************************/
...
void handle_rectangles(struct area_t * area) {
  ...
  #ifdef OMP
  #pragma omp parallel for shared(area,rect,curve) private(current_rectangle, current_left, current_height)
  #endif
  for(current_rectangle = 0; current_rectangle < rect->private_num_rect; current_rectangle++)
  {
    /* Calculate the x-value of the left side of the rectangle */
