...
void get_color_mpe(struct area_t * area) {
  struct display_t * d = &(area->d);
  #ifdef STRIPING
  int index, red=0, blue=0, green=0, id=0;
  ...
  blue = p_blue * MAX_RGB;
  index = MPE_Add_RGB_color(d->canvas,red,green,blue,d->colors);
  d->myColor = d->colors[index];
  #endif
}
