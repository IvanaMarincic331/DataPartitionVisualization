...
void	handle_rectangles_MPIstriping(struct area_t * area);
...
void area_under_curve(struct area_t * area, int argc, char ** argv){  
  ...
  #else
  handle_rectangles_MPIstriping(area); //works only for MPI (just striping)
  #endif
...
/************************************************************************************
 * Find boundaries of each rectangle and calculate the area for each using striping*
 ***********************************************************************************/
void handle_rectangles_MPIstriping(struct area_t * area){
  struct rec_t * rect = &(area->rect);
  struct curve_t * curve = &(area->curve);
  ...
  double current_left=0.0,current_height=0.0, rect_width = rect->width;
  
  for(current_rectangle = area->rank; current_rectangle < rect->num_rect; current_rectangle += area->numProcs)
  {
    /* Calculate the x-value of the left side of the rectangle */
    current_left = curve->xleft + current_rectangle *rect_width;
    ...
    area->my_sum += rect_width * current_height;
    
    /*Draw the current rectangle*/
    if(area->do_display){
      /*For circle and sine we will use the scaled width and height*/
      if(area->circle){
	rect_width = rect->circle_width;
	current_left = curve->xleft + current_rectangle*rect_width; 
	current_height = scale_circle(current_left);
      }
      if(area->sin){
	rect_width = rect->sin_width;
	current_left = curve->xleft + current_rectangle*rect_width;
	current_height = scale_sin(current_left);
      }
      draw_rectangle_mpe(area,current_left,current_height,rect_width);
    }
  }
  #endif
}
/*************************END OF handle_rectangles_MPI_striping()*****************************/
...
void sum_rectangles(struct area_t * area) {
  ...
  if (area->rank == 0){
    if(area->linear){
      ...
      if(area->curve.xright == UNIT) printf("Expected: 1/4 * %1.48f * radius^2\n",pi);
    }
    else if(area->sin){
      printf("%f\n",area->total_sum);
